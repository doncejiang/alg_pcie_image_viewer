...
ssize_t read_to_buffer(char *fname, int fd, char *buffer, uint64_t size_in,
			uint64_t base)
{
    ssize_t rc;
    int64_t count = 0;
	...
	if (count != size) {
        fprintf(stderr, "[FINAL]%s, R failed 0x%lx != 0x%lx.\n",
				fname, count, size);
		return -EIO;
	}
	return count;
...
ssize_t write_from_buffer(char *fname, int fd, char *buffer, uint64_t size_in,
			uint64_t base)
{
	ssize_t rc;
    int64_t count = 0;
